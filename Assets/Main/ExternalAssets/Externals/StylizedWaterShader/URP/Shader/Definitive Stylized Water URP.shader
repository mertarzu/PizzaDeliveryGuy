...
Shader "Definitive Stylized Water URP"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float FixCorrectDepth(float rawDepth, float _ZBufferParams)
			{
				float persp = LinearEyeDepth(rawDepth, _ZBufferParams);
				float ortho = (_ProjectionParams.z - _ProjectionParams.y) * (1 - rawDepth) + _ProjectionParams.y;
				return lerp(persp, ortho, unity_OrthoParams.w);
						 ...
						 ) : SV_TARGET
			{
				...
				float2 panner63 = ( mulTime64 * float2( 0.15,0.15 ) + texCoord62);
				float screenDepth67 = FixCorrectDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
